...
thomp.exp <- subset(allrefs,Effluent=="Exposure") # Chem variables .surf and .deep, thomp.exp only
  ...
  thomp.exp$NiSEMHFOfOC <- ((thomp.exp$SEMNi.surf/58.69)-(thomp.exp$ASCNi.surf/58.69))/(thomp.exp$C.avg/100)
...
####1. WATER-SED correlations
  ...
  a. Table of correlation coefficients between surface (rows 1-15) and deep (rows 16-30) sediment variables and water chemistry variables || Bold green cells represent significant correlations
...
library('formattable')
  p_bold <- formatter("span", style = x ~ style("font-weight" = ifelse(x < 0.05, "bold", NA)))
  
  
surf = surf[,c(6:18,21,22)]
deep = deep[,c(6:18,21,22)]
water = water [,5:12]
...
watersed.p = rbind(watersurf.p,waterdeep.p)
  ...
  highcor <- formatter("span",style = x ~ style(color = ifelse(watersed.p < 0.05, "green", "black"),
                                                "font-weight" = ifelse(watersed.p < 0.05, "bold", NA)))  
...
for(i in 1:ncol(water)) {
  ...
  highcor <- formatter("span",style = x ~ style(color = ifelse(surf.p < 0.05, "green", "black"),
                                                "font-weight" = ifelse(surf.p < 0.05, "bold", NA)))
  ...
  highcor <- formatter("span",style = x ~ style(color = ifelse(water.p < 0.05, "green", "black"),
                                                "font-weight" = ifelse(water.p < 0.05, "bold", NA)))
...
for(i in 1:ncol(r_ab)) {
  for(j in 1:ncol(allrefs)) {
    ...
    r_ab.cor[i,j] = signif(cor$estimate*10^2,2)
  }
}
...
library('formattable')
  ...
  style = x ~ style(color = ifelse(r_ab.p < 0.05, "green", "black"),"font-weight" = ifelse(r_ab.p < 0.05, "bold", NA)))
...
rda.water.sed=rda(chem.inv.hel[,66:103],chem.inv.hel[,c("TotalNi.surf","PAmFe.surf","Cond","pH")])
  #anova(rda.water.sed)
...
plot(REL_EPH~SEMNi.surf.mol, data=chem.inv.exp[c(1,3:11,13:20),],log="x",pch=c(rep(16,9),rep(1,9)),lwd=3,las=1,cex=1.5,
     ...
     xlab=expression(paste("Ni"[SEM]*" (µmol g"^"-1",")")),ylab="Ephemeridae abundance (%)",xaxt="n")
logaxis(-1,1,1)
...
plot(REL_EPH~NiSEMHFOfOC, data=chem.inv.exp[c(1,3:11,13:20),],pch=c(rep(16,9),rep(1,9)),log="x",lwd=3,las=1,cex=1.5,
     ...
     xlab=expression(paste("(Ni"[SEM]*"-Ni"[HFO]*")/",ƒ,""[OC]*" (µmol g"^"-1",")")),
     ylab="",yaxt="n",xaxt="n")
axis(2,at=c(0,10,20,30,40,50),labels=c("","","","","",""),tick=T)
logaxis(0,2,1)
...
plot(REL_TAL~TotalNi.surf,log="x",data=chem.inv[1:20,], xlim=c(40,600), ylim=c(0,45), cex=1.5, col=2,
     pch=16,las=1,xlab=expression(paste("Ni"[TOT]*" (mg kg"^"-1",")")),ylab="Talitridae relative abundance (%)") 
      #Ref values are all the average of the 4 ref sites from the respective mine
points(REL_TAL~TotalNi.surf,data=chem.inv[21:40,],cex=1.5,pch=16,las=1, xlab="n",ylab="n")

legend("topleft", c("Birch", "Thompson"), text.col=c(2,1), bty="n", text.font=2)

CRM=nls(REL_TAL~a/(1+10^(2*b*(log10(TotalNi.surf)-c))),start=list(a=50,b=2,c=3),data=chem.inv)
...
plot(REL_TAL~SEMNi.surf,log="x",data=chem.inv[1:20,],cex=1.5, xlim=c(15,510), ylim=c(0,45), col=2,
     pch=16,las=1,xlab=expression(paste("Ni"[SEM]*" (mg kg"^"-1",")")),ylab="Talitridae relative abundance (%)") #Ref values are all the average of the 4 ref sites from the respective mine
points(REL_TAL~SEMNi.surf,data=chem.inv[21:40,],cex=1.5,pch=16,las=1, xlab="n",ylab="n")
...
plot(REL_TAL~SEMAVSfOC.avg,log="x",data=chem.inv[1:20,], xlim=c(5,120), ylim=c(0,45),cex=1.5, col=2,
     pch=16,las=1,xlab=expression(paste("(Ni"[SEM]*"-AVS)/",ƒ,""[OC]*" (µmol g"^"-1",")")),ylab="Talitridae relative abundance (%)") #Ref values are all the average of the 4 ref sites from the respective mine
...
plot(REL_TAL~NiSEMHFOfOC,log="x",data=chem.inv[1:20,],xlim=c(150,5000), ylim=c(0,45), cex=1.5, col=2,
     pch=16,las=1,xlab=expression(paste("(Ni"[SEM]*"-Ni"[HFO]*")/",ƒ,""[OC]*" (mg kg"^"-1",")")),ylab="Talitridae relative abundance (%)") #Ref values are all the average of the 4 ref sites from the respective mine
points(REL_TAL~NiSEMHFOfOC,data=chem.inv[21:40,],cex=1.5,pch=16,las=1, xlab="n",ylab="n")

legend("topleft", c("Birch", "Thompson"), text.col=c(2,1), bty="n", text.font=2)

CRM=nls(REL_TAL~a/(1+10^(3*b*(log10(NiSEMHFOfOC)-c))),start=list(a=50,b=10,c=3.5),data=chem.inv)
